      ...
      mcpServer: {
        start: async function() {
          ...
          try {
            ...
            async function listEvents(startDate, endDate, maxResults, calendarId) {
              if (!cal) {
                return { error: "Calendar not available" };
              }
              try {
                ...
                // Convert ISO → Thunderbird format
                function toCalDateTime(str) {
                  const clean = str.replace(/[-:]/g, "").split(".")[0];
                  return cal.createDateTime(clean);
                }

                const rangeStart = startDate ? toCalDateTime(startDate) : cal.dtz.now();
                if (!rangeStart) return { error: `Invalid startDate: ${startDate}` };
                const rangeEnd = endDate ? toCalDateTime(endDate) : cal.dtz.getDateTime(rangeStart, 30 * 86400000);
                if (!rangeEnd) return { error: `Invalid endDate: ${endDate}` };
                ...
                // Some providers honor ITEM_FILTER_CLASS_OCCURRENCES and return
                // expanded occurrences; others silently return only the master
                // recurring item. Detect the latter and expand manually so
                // recurring events are never dropped.
                function expandMasterIfNeeded(item) {
                  if (item && item.recurrenceInfo && !item.recurrenceId) {
                    try {
                      const occ = item.recurrenceInfo.getOccurrences(
                        rangeStart,
                        rangeEnd,
                        0,
                        {}
                      );
                      if (occ && occ.length) return occ;
                    } catch { /* fall through and keep master */ }
                  }
                  return [item];
                }
                ...
                for (const calendar of targets) {
                  ...
                  try {
                    ...
                    if (typeof calendar.getItemsAsArray === "function") {
                      items = await calendar.getItemsAsArray(filter, 0, rangeStart, rangeEnd);
                    } else {
                      items = [];
                      const stream = cal.iterate.streamValues(calendar.getItems(filter, 0, rangeStart, rangeEnd));
                      for await (const chunk of stream) {
                        for (const i of chunk) items.push(i);
                      }
                    }
                  } catch {
                    // Fallback: fetch without occurrence expansion. We rely on
                    // expandMasterIfNeeded below to materialize occurrences.
                    items = [];
                    const stream = cal.iterate.streamValues(
                      calendar.getItems(
                        Ci.calICalendar.ITEM_FILTER_TYPE_EVENT,
                        0,
                        rangeStart,
                  ...
                  outer: for (const item of items) {
                    // Always run through master-expansion: this is a no-op for
                    // already-expanded occurrences (they carry recurrenceId)
                    // and for non-recurring items.
                    for (const occ of expandMasterIfNeeded(item)) {
                      if (results.length >= EVENT_COLLECTION_CAP) break outer;
            ...
            async function callTool(name, args) {
              switch (name) {
                ...
                case "listEvents":
                  return await listEvents(args.startDate, args.endDate, args.maxResults, args.calendarId);
                case "updateEvent":
